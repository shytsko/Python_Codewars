...
class User:
    ...
    def inc_progress(self, completed_task_rank):
        if completed_task_rank < -8 or completed_task_rank > 8 or completed_task_rank == 0:
            raise Exception
        if self.rank == 8:
            return
        d = completed_task_rank - self.rank
        if self.rank < 0 < completed_task_rank:
            d -= 1
        ...
        if self.progress >= 100:
            d_rank = self.progress // 100
            if self.rank < 0 <= (self.rank + d_rank):
                d_rank += 1
            self.rank += d_rank
            self.progress %= 100
...
user.inc_progress(1)
print(f"{user.rank=} {user.progress=}")
user.inc_progress(1)
print(f"{user.rank=} {user.progress=}")
user.inc_progress(1)
print(f"{user.rank=} {user.progress=}")
